...
class BaseTrackerPredictor(nn.Module):
    def __init__(
        ...
        self.adapter = Adapter(dim_in=self.latent_dim, dim_out=self.latent_dim)
    ...
    def forward(self, query_points, fmaps=None, iters=6, return_feat=False, down_ratio=1, apply_sigmoid=True):
        ...
        for _ in range(iters):
            ...
            track_feats_ = self.ffeat_updater(self.ffeat_norm(delta_feats_)) + track_feats_ + self.adapter(delta_feats_)
