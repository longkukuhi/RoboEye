...
class ArmbenchDataset(Beit3baseDataset):
    def __init__(
        ...
        self.args = args
        if args.task in ['armbench3t1', 'armbench3t1extract', 'armbench3t1sample']:
        ...
        elif args.task in ['armbenchpick1', 'armbenchpick1extract', 'armbenchpick1sample',
    ...
    def _get_pick_image(self, pickid):
        if self.args.task in ['armbench3t1', 'armbench3t1extract', 'armbench3t1sample']:
            ...
            return images
        elif self.args.task in ['armbenchpick1', 'armbenchpick1extract', 'armbenchpick1sample', 'armbenchpick1_clloss', 'armbenchpick1_nearestref']:
    ...
    def _get_ref_image(self, idx):
        if self.args.task in ['armbench3t1', 'armbench3t1extract', 'armbench3t1sample', 'armbenchpick1', 
                              'armbenchpick1extract', 'armbenchpick1sample', 'armbenchpick1_clloss']:
            pickid = idx
    ...
    def _get_image_text_example(self, index: int, data: dict):
        ...
        elif self.args.task in ['armbenchpick1', 'armbenchpick1extract', 'armbenchpick1sample']:
...
class ArmbenchPickDataset(Beit3baseDataset):
        ...
        if args.task in ['armbench3t1', 'armbench3t1extract', 'armbench3t1sample']:
            with open(os.path.join(data_path,  f'{split}_data_3t1.json'), 'r') as f:
                self.pick_items = json.load(f)
                           ...
                           'armbenchpick1to1', 'armbenchpick1_clloss', 'armbenchpick1_nearestref']:
            with open(os.path.join(data_path,  f'all_{split}_data.json'), 'r') as f:
                self.pick_items = json.load(f)
    ...
    def _get_image_path(self, pickid):
        if self.args.task in ['armbench3t1', 'armbench3t1extract', 'armbench3t1sample']:
            image_paths = self.pick_items[pickid]['pickimg_3t1_paths']
            pick_image_paths = [os.path.join(self.data_path, 'Picks', pickid, image_path) for image_path in image_paths]
            return pick_image_paths
        elif self.args.task in ['armbenchpick1', 'armbenchpick1extract', 'armbenchpick1sample', 
                                'armbenchpick1to1', 'armbenchpick1_clloss', 'armbenchpick1_nearestref']:
            image_path = os.path.join(self.data_path, 'Picks', pickid, 'PickRGB.jpg')
            return image_path
        else:
            raise NotImplementedError
...
class ArmbenchRefDataset(Beit3baseDataset):
    ...
    def _get_image_path(self, index):
        ...
        img_path = os.path.join(self.data_path, 'Reference_Images',  self.all_img_paths[index])
        return img_path
...
class ArmbenchRefDatasetAllRef(Beit3baseDataset):
    ...
    def _get_image_path(self, index):
        image_path = os.path.join(self.data_path, 'Reference_Images',  self.all_img_paths[index])
        return image_path
...
def creat_ref_dataset(args, split):
    ...
    dataset = ArmbenchRefDatasetAllRef(
        data_path=args.data_path,
        split=split,
        transform=transform,
        tokenizer=tokenizer,
