...
from armbench.armbench_datasets import create_armbench_dataset, creat_query_dataset, creat_ref_dataset
...
from armbench.engine import train_one_epoch, get_handler, evaluate
...
def get_args():
    ...
    parser.add_argument('--task', type=str, required=True,
                        choices=['armbench3t1', 'armbench3t1extract', 'armbench3t1sample', 'armbenchpick1',
                                 'armbenchpick1extract', 'armbenchpick1sample', 'armbenchpick1to1', 
    ...
    parser.add_argument('--classifier_path', default='',
                        help="classifier's checkpoint path")
...
def main(args, ds_init):
    ...
    data_loader_train = create_armbench_dataset(args, split="train")
    # data_loader_val = create_armbench_dataset(args, split="test")
    ...
    if not args.model.endswith(args.task):
        if args.task in ['armbench3t1', 'armbench3t1extract', 'armbench3t1sample', 'armbenchpick1',
                         'armbenchpick1extract', 'armbenchpick1sample', "armbenchpick1to1", "armbenchpick1_clloss",
    ...
    if utils.is_main_process() and args.enable_wandb:
        ...
        wandb.init(project='armbench3t1',
    ...
    if args.vggt_path:
        if args.task in ['armbenchpick1extract', 'armbench3t1extract']:
            vggt_state = torch.load(args.vggt_path, map_location="cpu", weights_only=False)
            vggt_state = {k.replace("vggt.", ""): v for k, v in vggt_state.items()}
            missing, unexpect = task_handler.vggt.load_state_dict(vggt_state, strict=False)
            print(f"vggt missing {missing}, unexpected {unexpect}") 

        if args.task in ['armbenchpick1sample', 'armbench3t1sample']:
            checkpoint = torch.load(args.vggt_path, map_location="cpu", weights_only=False)
            vggt_state = checkpoint["model"]
            vggt_state = {k.replace("vggt.", ""): v for k, v in vggt_state.items()}
            missing, unexpect = task_handler.vggt.load_state_dict(vggt_state, strict=False)
            print(f"vggt missing {missing}, unexpected {unexpect}") 

    if args.classifier_path:
        checkpoint = torch.load(args.classifier_path, map_location="cpu", weights_only=False)
        classifier_state = checkpoint["model"]
        missing, unexpect = task_handler.classifier.load_state_dict(classifier_state, strict=False)
        print(f"classifier missing {missing}, unexpected {unexpect}") 

    # mixup for imagenet
    mixup_fn = None

    query_dataloader = creat_query_dataset(args, 'test')
