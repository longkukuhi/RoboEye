...
def load_model(vggt_ckpt_path, model):
    vggt_state = torch.load(vggt_ckpt_path, map_location="cpu")
    #vggt_state = {k.replace("module.", ""): v for k, v in vggt_state.items()} 
    vggt_state = vggt_state["model"] if "model" in vggt_state else vggt_state
    
    missing_key, unexpected_key = model.vggt.load_state_dict(vggt_state, strict=False)
    return missing_key
